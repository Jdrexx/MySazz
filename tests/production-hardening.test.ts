...
async function register(app) {
  return request(app)
    ...
    .send(registrationPayload("alice", "alice@example.com"));
}
...
test("sets profile avatar and cover images from authenticated uploads", async () => {
  ...
  const avatar = await request(app)
    .post("/api/uploads")
    .set("Cookie", cookie)
    .attach("media", Buffer.from("avatar image"), {
      filename: "avatar.png",
  ...
  const avatarSet = await request(app)
    .post("/api/me/avatar")
    .set("Cookie", cookie)
    .send({ media_id: avatar.body.media.id });
  assert.equal(avatarSet.status, 200);
  assert.equal(avatarSet.body.user.avatar_url, avatar.body.media.url);

  const coverSet = await request(app)
    .post("/api/me/cover")
    .set("Cookie", cookie)
    .send({ media_id: cover.body.media.id });
...
test("production config rejects weak secrets and missing persistent database", () => {
  assert.throws(
    () => getRuntimeConfig({ NODE_ENV: "production", JWT_SECRET: "short" }),
    /JWT_SECRET/,
  );
  assert.throws(
    () =>
      getRuntimeConfig({ NODE_ENV: "production", JWT_SECRET: "x".repeat(40) }),
