...
async function register(app, username, email) {
  const res = await request(app)
    ...
    .send(registrationPayload(username, email));
...
test("Socket.IO relays video-call and WebRTC signaling events between users", async () => {
  ...
  const connection = await request(app)
    .post("/api/connections/bob_video/request")
  ...
  try {
    await Promise.all([
      once(aliceSocket, "connect"),
      once(bobSocket, "connect"),
    ]);

    const incoming = once(bobSocket, "video:incoming");
    const ack = await new Promise((resolve) =>
      aliceSocket.emit("video:call", { recipientId: bob.user.id }, resolve),
    );
    ...
    assert.deepEqual((await iceReceived).candidate, {
