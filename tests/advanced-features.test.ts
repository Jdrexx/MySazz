...
test("creates notifications for likes, comments, follows and marks them read", async () => {
  ...
  const post = await request(app)
    ...
    .send({ body: "Notify me" });
  await request(app)
    .post(`/api/posts/${post.body.post.id}/like`)
    .set("Cookie", bobCookie);
  await request(app)
    .post(`/api/posts/${post.body.post.id}/comments`)
    .set("Cookie", bobCookie)
    .send({ body: "Nice" });
  ...
  assert.deepEqual(notifications.body.notifications.map((n) => n.type).sort(), [
    "comment",
    "follow",
    "like",
  ]);
  ...
  const read = await request(app)
    .post(`/api/notifications/${notifications.body.notifications[0].id}/read`)
    .set("Cookie", aliceCookie);
...
test("searches users and posts", async () => {
  ...
  await request(app)
    ...
    .send({ body: "Learning SQLite search" });

  const res = await request(app)
    .get("/api/search?q=sqlite")
    .set("Cookie", alice.headers["set-cookie"]);
...
test("supports password reset and email verification token flows", async () => {
  ...
  const resetConfirm = await request(app)
    ...
    .send({ token: resetRequest.body.dev_token, password: "NewPassword123!" });
  ...
  const verifyConfirm = await request(app)
    .post("/api/auth/email-verification/confirm")
    .send({ token: verifyRequest.body.dev_token });
...
test("first registered user can moderate reports and remove posts", async () => {
  ...
  const report = await request(app)
    .post(`/api/reports/posts/${post.body.post.id}`)
  ...
  const removed = await request(app)
    .delete(`/api/admin/posts/${post.body.post.id}`)
    .set("Cookie", adminCookie);
  assert.equal(removed.status, 200);
});

test("sends chat messages and realtime notifications are delivered over socket.io", async () => {
  const { app } = setup();
  const alice = await register(app, "alice", "alice@example.com");
  const bob = await register(app, "bob", "bob@example.com");
  const aliceCookie = alice.headers["set-cookie"];
  ...
  const connection = await request(app)
    .post("/api/connections/bob/request")
    .set("Cookie", aliceCookie);
  await request(app)
  ...
  const thread = await request(app)
    .get("/api/messages/bob")
    .set("Cookie", aliceCookie);
  assert.equal(thread.status, 200);
  assert.equal(thread.body.messages[0].body, "Hello Bob");

  // No SSE stub: realtime is socket.io only, so the dead /api/messages/stream
  // endpoint returns 404 instead of a fake one-shot stream.
