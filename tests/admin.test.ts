...
test("admin can list, edit, suspend, and unsuspend users", async () => {
  ...
  const suspend = await request(app)
    .post(`/api/admin/users/${user1.id}/suspend`)
    .set("Cookie", adminCookie);
  ...
  const unsuspend = await request(app)
    .post(`/api/admin/users/${user1.id}/unsuspend`)
    .set("Cookie", adminCookie);
...
test("admin can list all posts including hidden ones", async () => {
  ...
  await request(app)
    ...
    .send({ body: "Visible post" });
  const hidden = await request(app)
    .post("/api/posts")
    .set("Cookie", userCookie)
    .send({ body: "Hidden post" });
  await request(app)
    .delete(`/api/admin/posts/${hidden.body.post.id}`)
    .set("Cookie", cookie);
  ...
  const hiddenFilter = await request(app)
    .get("/api/admin/posts?hidden=1")
    .set("Cookie", cookie);
...
test("admin can resolve and dismiss reports", async () => {
  ...
  const resolved = await request(app)
    .post(`/api/admin/reports/${reports.body.reports[0].id}/resolve`)
    .set("Cookie", cookie);
  ...
  const post2 = await request(app)
    ...
    .send({ body: "Another" });
  await request(app)
    .post(`/api/reports/posts/${post2.body.post.id}`)
    .set("Cookie", cookie)
    .send({ reason: "not spam" });
  const reports2 = await request(app)
    .get("/api/admin/reports")
    .set("Cookie", cookie);
  ...
  const dismissed = await request(app)
    .post(`/api/admin/reports/${openReport.id}/dismiss`)
    .set("Cookie", cookie);
...
test("admin can create a specific user manually", async () => {
  ...
  const dup = await request(app)
    .post("/api/admin/users")
    .set("Cookie", cookie)
    .send({ username: "newuser", email: "other@test.com" });
