...
test("bookmark and unbookmark post", async () => {
  ...
  const add = await request(app)
    .post(`/api/bookmarks/${post.body.post.id}`)
    .set("Cookie", c);
...
test("change password", async () => {
  ...
  const login = await request(app)
    .post("/api/auth/login")
    .send({ email: "a@t.com", password: "Password123!" });
  assert.equal(login.status, 401);

  // New password works
  const login2 = await request(app)
    .post("/api/auth/login")
    .send({ email: "a@t.com", password: "NewPass1234!" });
...
test("admin can edit post content", async () => {
  ...
  const edit = await request(app)
    .patch(`/api/admin/posts/${post.body.post.id}`)
    .set("Cookie", ac)
...
test("feed pagination", async () => {
  ...
  const page2 = await request(app)
    .get(`/api/posts?before=${page1.body.next}&limit=3`)
    .set("Cookie", c);
