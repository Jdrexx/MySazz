...
test("requires all MySazz eligibility and privacy attestations", async () => {
  const { app } = setup();
  const missingConsent = registrationPayload(
    "alice_consent",
    "alice.consent@example.com",
  );
  ...
  assert.deepEqual(accepted.body.user.member_profile.connection_intents, [
    "friendship",
    "peer_support",
  ]);
});
...
test("keeps sensitive profile fields private until the member opts in", async () => {
  ...
  const hiddenSearch = await request(app)
    .get("/api/search?q=loss_and_grief")
    .set("Cookie", bobCookie);
  assert.equal(
    hiddenSearch.body.users.some((user) => user.username === "alice_private"),
    false,
  );
  ...
  await request(app)
    .patch("/api/me")
    .set("Cookie", aliceCookie)
    .send({ show_relationship_status: true, show_experience_tags: true });
  const visible = await request(app)
    .get("/api/users/alice_private")
    .set("Cookie", bobCookie);
  assert.equal(visible.body.user.member_profile.relationship_status, "single");
  assert.deepEqual(visible.body.user.member_profile.experience_tags, [
    "loss_and_grief",
  ]);
  ...
  const visibleSearch = await request(app)
    ...
    .set("Cookie", bobCookie);
  assert.equal(
    visibleSearch.body.users.some((user) => user.username === "alice_private"),
    true,
  );
});
...
test("requires authentication for member stories, discovery, and media", async () => {
  ...
  const upload = await request(app)
    .post("/api/uploads")
    .set("Cookie", cookie)
    .attach("media", Buffer.from("private image"), {
  ...
  assert.equal(
    (await request(app).get(upload.body.media.url).set("Cookie", cookie))
      .status,
    200,
  );
});
...
test("gates encrypted messages behind mutual connections and propagates blocks", async () => {
  ...
  const connection = await request(app)
    .post("/api/connections/bob_safe/request")
    .set("Cookie", aliceCookie);
  assert.equal(connection.status, 201);
  assert.equal(
    (
      await request(app)
        .post(`/api/connections/${connection.body.id}/respond`)
        .set("Cookie", bobCookie)
        .send({ action: "accept" })
    ).status,
    200,
  ...
  const thread = await request(app)
    .get("/api/messages/alice_safe")
    .set("Cookie", bobCookie);
  assert.equal(thread.body.messages[0].body, "A private hello");

  await request(app)
    .post(`/api/blocks/${bob.body.user.id}`)
    .set("Cookie", aliceCookie);
  assert.equal(
    (
      await request(app)
  ...
  assert.equal(
    await db.get("SELECT id FROM connections WHERE id = ?", connection.body.id),
    null,
  );
  const feed = await request(app).get("/api/posts").set("Cookie", aliceCookie);
  assert.equal(
    feed.body.posts.some((post) => post.username === "bob_safe"),
    false,
  );
  const search = await request(app)
    .get("/api/search?q=bob_safe")
    .set("Cookie", aliceCookie);
...
test("supports authenticator-app two-factor login", async () => {
  ...
  const pending = await request(app)
    .post("/api/auth/login")
    .send({ email: "alice.mfa@example.com", password: "Password123!" });
  ...
  const login = await request(app)
    ...
    .send({
      ...
      mfa_code: totpCode(setupResponse.body.secret),
    });
...
test("uses configured production admins and gates unverified member actions", async () => {
  ...
  const config = {
    ...getRuntimeConfig({
      NODE_ENV: "test",
      ADMIN_EMAILS: "owner@mysazz.com",
      BOOTSTRAP_FIRST_USER_ADMIN: "false",
    }),
  ...
  const first = await request(app)
    .post("/api/auth/register")
    .send(registrationPayload("first_member", "first@example.com"));
  ...
  assert.equal(
    (
      await request(app)
        ...
        .send({ body: "Before verification" })
    ).status,
    403,
  );
  await request(app)
    .post("/api/auth/email-verification/confirm")
    .send({ token: first.body.verification.dev_token });
  assert.equal(
    (
      await request(app)
        .post("/api/posts")
        .set("Cookie", first.headers["set-cookie"])
        .send({ body: "After verification" })
    ).status,
    201,
  );
  ...
  const owner = await request(app)
    .post("/api/auth/register")
    .send(registrationPayload("owner", "owner@mysazz.com"));
