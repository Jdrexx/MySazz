...
export default function AdminPage() {
  ...
  const loadLog = useCallback(async () => {
    try {
      setLog((await api("/api/admin/log?limit=100")).entries);
    } catch {
      setLog([]);
    }
  }, []);
  ...
  async function bulkAct(action) {
    const idsMap = {
      suspend_users: selectedUsers,
      unsuspend_users: selectedUsers,
      delete_users: selectedUsers,
      hide_posts: selectedPosts,
      unhide_posts: selectedPosts,
      delete_posts: selectedPosts,
    };
  ...
  if (!user || !stats) {
    return (
      <main className="shell">
        <p className="status">Loading admin panel...</p>
      </main>
    );
  }
        ...
        <div className="inline seedButtons">
          {[1, 5, 10, 15, 20].map((n) => (
            <button
              key={n}
              onClick={() =>
                adminAct("POST", "/api/admin/seed/users", { count: n })
              }
            >
              +{n}
            </button>
      ...
      {tab === "users" && (
        <section className="adminSection">
          <div className="inline" style={{ marginBottom: ".5rem" }}>
            ...
            <button
              className="secondaryButton"
              onClick={() => bulkAct("unsuspend_users")}
              disabled={selectedUsers.size === 0}
            >
              Unsuspend selected
            </button>
                  ...
                  <th>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedUsers(
                          new Set(e.target.checked ? all.map((u) => u.id) : []),
                        );
                      }}
                    />
                  </th>
                    ...
                    <tr key={u.id}>
                      <td>
                        <input
                          ...
                          onChange={() =>
                            toggleSelect(selectedUsers, setSelectedUsers, u.id)
                          }
                        />
                      </td>
                      <td>{u.id}</td>
                      ...
                      <td className="adminActions">
                        <button
                          onClick={() =>
                            adminAct("PATCH", `/api/admin/users/${u.id}`, {
                              is_admin: !u.is_admin,
                            })
                          }
                        >
                        ...
                        <button
                          onClick={() =>
                            adminAct(
                              "POST",
                              `/api/admin/users/${u.id}/${u.is_suspended ? "unsuspend" : "suspend"}`,
                            )
                          }
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (confirm(`Delete @${u.username}?`))
                              adminAct("DELETE", `/api/admin/users/${u.id}`);
                          }}
                        >
                          Delete
                        </button>
                  ...
                  <tr key={p.id} className={p.is_hidden ? "rowHidden" : ""}>
                    <td>
                      <input
                        ...
                        onChange={() =>
                          toggleSelect(selectedPosts, setSelectedPosts, p.id)
                        }
                      />
                    </td>
                    <td>{p.id}</td>
                    <td>@{p.username}</td>
                    <td className="postBodyCell">
                      {p.body.slice(0, 80)}
                      {p.body.length > 80 ? "…" : ""}
                    </td>
                    <td>{p.like_count}</td>
                    ...
                    <td className="adminActions">
                      <button
                        onClick={() =>
                          adminAct("DELETE", `/api/admin/posts/${p.id}`)
                        }
                      ...
                      <button
                        onClick={() =>
                          adminAct("POST", `/api/admin/posts/${p.id}/unhide`)
                        }
                        disabled={!p.is_hidden}
                      >
                        Unhide
                        ...
                        <>
                          ...
                          <button
                            onClick={() =>
                              adminAct(
                                "POST",
                                `/api/admin/reports/${r.id}/dismiss`,
                              )
                            }
                          >
                            Dismiss
