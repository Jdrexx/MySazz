...
export default function Home() {
  ...
  const loadFeed = useCallback(async (cursor) => {
    const url = cursor
      ? `/api/posts?before=${cursor}&limit=25`
      : "/api/posts?limit=25";
  ...
  useEffect(() => {
    ...
    socket.on("message:new", ({ message }) => {
      if (
        chatUser &&
        [message.sender_username, message.recipient_username].includes(chatUser)
      ) {
    ...
    <main className="shell">
      <section className="hero">
        <div className="heroCopy">
          <p className="eyebrow">
            Your story. Your connections. Your next chapter.
          </p>
          <h1>A place to move forward—together.</h1>
          <p className="heroLead">
            Meet adults who understand that lived experience is part of your
            story, not the whole of who you are.
          </p>
          <div
            className="heroPromises"
            aria-label="MySazz community principles"
          >
          ...
          {!user && (
            <a className="primaryLink" href="#member-access">
              Join the community
            </a>
          )}
        </div>
        <figure className="heroVisual">
          <img
            src="/brand/mysazz-community-hero.webp"
            alt="Four adults gathered around a table in a welcoming community space"
          />
          <figcaption>
            Connection begins with being seen as a whole person.
          </figcaption>
        </figure>
      </section>
        ...
        <>
          {!user.email_verified && (
            <section className="noticeCard" role="status">
              ...
              <span>
                Check your inbox before relying on MySazz member features.
              </span>
              {verification?.dev_token && (
                <a
                  href={`/verify-email?token=${encodeURIComponent(verification.dev_token)}`}
                >
