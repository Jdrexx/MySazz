...
export async function attachRealtimeServer(
  httpServer,
  { db, jwtSecret, config = null as any },
) {
  const publicUrl = config?.publicUrl || process.env.PUBLIC_URL || true;
  const io = new Server(httpServer, {
    cors: {
      // Production requires an https PUBLIC_URL (enforced in lib/env.ts), so
      // this is an explicit origin list, never the reflect-any `true` fallback.
      origin: publicUrl === true ? true : [publicUrl],
      credentials: true,
    },
  });
  ...
  io.use(async (socket, next) => {
    ...
    if (
      ...
      (config?.requireEmailVerification && !user.email_verified)
    ) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("typing:start", async ({ recipientId } = {}) => {
      ...
      if (recipient)
        socket
          ...
          .emit("typing:start", { userId: user.id, username: user.username });
    });
    ...
    socket.on("typing:stop", async ({ recipientId } = {}) => {
      ...
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("typing:stop", { userId: user.id, username: user.username });
    });
    ...
    socket.on("video:call", async ({ recipientId } = {}, ack) => {
      ...
      io.to(`user:${recipient.id}`).emit("video:incoming", {
        caller: publicUser,
      });
    ...
    socket.on("video:accept", async ({ recipientId } = {}) => {
      ...
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("video:accepted", { by: publicUser });
    });
    ...
    socket.on("video:reject", async ({ recipientId } = {}) => {
      ...
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("video:rejected", { by: publicUser });
    });
    ...
    socket.on("video:end", async ({ recipientId } = {}) => {
      ...
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("video:ended", { by: publicUser });
    });

    socket.on("webrtc:offer", async ({ recipientId, description } = {}) => {
      const recipient = description
        ? await getRecipient(db, user.id, recipientId)
        : null;
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("webrtc:offer", { from: publicUser, description });
    });
    ...
    socket.on("webrtc:answer", async ({ recipientId, description } = {}) => {
      ...
      if (recipient)
        socket
          .to(`user:${recipient.id}`)
          .emit("webrtc:answer", { from: publicUser, description });
    });

    socket.on(
      "webrtc:ice-candidate",
      async ({ recipientId, candidate } = {}) => {
        ...
        if (recipient)
          socket
            .to(`user:${recipient.id}`)
            .emit("webrtc:ice-candidate", { from: publicUser, candidate });
      },
    );
  });
