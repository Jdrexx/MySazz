...
export function checkLoginRate(email) {
  ...
  if (entry.count >= MAX_LOGIN_ATTEMPTS) {
    const retryAfter = Math.ceil(
      (LOGIN_WINDOW_MS - (Date.now() - entry.firstAttempt)) / 60000,
    );
...
export function memberRequired(req, res, next) {
  ...
  if (!req.user.onboarding_complete)
    return res.status(403).json({
      error: "Complete MySazz onboarding before using member features",
      code: "ONBOARDING_REQUIRED",
    });
  const requireVerification =
    req.app.locals.context?.config?.requireEmailVerification;
  if (requireVerification && !req.user.email_verified) {
    return res.status(403).json({
      error: "Verify your email before using member features",
...
export function uploadsAuth(req, res, next) {
  if (!req.user)
    return res
      .status(401)
      .json({ error: "Authentication required to access uploads" });
