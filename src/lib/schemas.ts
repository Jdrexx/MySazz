...
export const registerSchema = z.object({
  ...
  connection_intents: z
    .array(connectionIntent)
    .min(1)
    .max(CONNECTION_INTENTS.length),
  experience_tags: z
    .array(experienceTag)
    .max(EXPERIENCE_TAGS.length)
    .default([]),
  city: z.string().trim().max(100).default(""),
  region: z.string().trim().max(100).default(""),
  age_18_plus: z.literal(true, {
    errorMap: () => ({ message: "You must confirm that you are 18 or older" }),
  }),
  recovery_one_year: z.literal(true, {
    errorMap: () => ({
      message:
        "MySazz currently requires at least one year of self-attested recovery progress",
    }),
  }),
  member_confidentiality: z.literal(true, {
    errorMap: () => ({
      message: "You must agree to protect member privacy and confidentiality",
    }),
  }),
  ...
  not_medical_care: z.literal(true, {
    errorMap: () => ({
      message:
        "You must acknowledge that MySazz does not replace professional care",
    }),
  }),
});
