...
export function serializeMemberProfile(profile, own = false) {
  ...
  const serialized: any = {
    relationship_status:
      profile.show_relationship_status || own
        ? profile.relationship_status
        : "",
    connection_intents: parseStringArray(profile.connection_intents),
    experience_tags:
      profile.show_experience_tags || own
        ? parseStringArray(profile.experience_tags)
        : [],
    city: profile.city,
...
export async function ownMember(db, user) {
  ...
  return {
    ...
    member_profile: serializeMemberProfile(
      await getMemberProfile(db, user.id),
      true,
    ),
  };
}
