...
export function createConnectionsRouter({ db }) {
  ...
  router.get("/connections", memberRequired, async (req, res) => {
    ...
    for (const row of rows) {
      if (await isBlockedEitherWay(db, row.requester_id, row.recipient_id))
        continue;
  ...
  router.get(
    "/connections/status/:username",
    memberRequired,
    async (req, res) => {
      const target = await db.get(
        "SELECT id FROM users WHERE username = ? AND is_suspended = 0",
        req.params.username,
      );
      if (!target || (await isBlockedEitherWay(db, req.user.id, target.id)))
  ...
  router.post(
    ...
    `,
      ...
      if (
        ...
        (await isBlockedEitherWay(db, req.user.id, target?.id))
      ) {
        return res.status(404).json({ error: "User not found" });
      }
    ...
    `,
      ...
      if (existing)
        await db.run("DELETE FROM connections WHERE id = ?", existing.id);
      const result = await db.run(
        "INSERT INTO connections (requester_id, recipient_id) VALUES (?, ?)",
        req.user.id,
        target.id,
      );
      ...
      res.status(201).json({
        ...
        member: await connectionMember(target),
      });
    },
  );
  ...
  router.post("/connections/:id/respond", memberRequired, async (req, res) => {
    ...
    const connection = await db.get(
      "SELECT * FROM connections WHERE id = ? AND recipient_id = ? AND status = ?",
      req.params.id,
      req.user.id,
      "pending",
    );
