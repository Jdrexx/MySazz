...
export function createPostsRouter({ db }) {
  ...
  async function visiblePost(req, id) {
    const post = await db.get(
      "SELECT * FROM posts WHERE id = ? AND is_hidden = 0",
      id,
    );
  ...
  router.get("/feed", memberRequired, async (req, res) => {
    ...
    if (before) {
      ...
      if (beforeId && beforeDate) {
        ...
        const next = hasMore
          ? `${posts[posts.length - 1].id}_${posts[posts.length - 1].created_at.replace(" ", "T")}`
          : null;
  ...
  router.post("/posts", memberRequired, async (req, res) => {
    ...
    const result = await db.run(
      "INSERT INTO posts (user_id, body, image_url, media_id) VALUES (?, ?, ?, ?)",
      req.user.id,
      parsed.data.body,
  ...
  router.patch("/posts/:id", memberRequired, async (req, res) => {
    ...
    if (!body || body.length > 500)
      return res
        .status(400)
        .json({ error: "Post body must be 1-500 characters" });
    await db.run(
    ...
    const updated = (
      await getPosts(db, req.user.id, "WHERE posts.id = ?", [post.id], 1)
    )[0];
  ...
  router.post("/posts/:id/comments", memberRequired, async (req, res) => {
    ...
    await createNotification(db, {
      ...
      entityType: "post",
      entityId: post.id,
      body: `${req.user.username} commented on your story`,
    });
  ...
  router.get("/bookmarks", memberRequired, async (req, res) => {
    const bookmarkRows = await db.all(
