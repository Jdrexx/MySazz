...
export function createMessagesRouter({ db, config }) {
  ...
  router.get("/messages/:username", memberRequired, async (req, res) => {
    ...
    if (!(await areConnected(db, req.user.id, other.id)))
      return res
        .status(403)
        .json({ error: "An accepted connection is required before messaging" });
  ...
  router.post("/messages/:username", memberRequired, async (req, res) => {
    ...
    if (!(await areConnected(db, req.user.id, recipient.id)))
      return res
        .status(403)
        .json({ error: "An accepted connection is required before messaging" });

    const encryptedBody = encryptSensitive(
      parsed.data.body,
