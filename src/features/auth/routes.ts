...
export function createAuthRouter({ db, jwtSecret, config, email }) {
  ...
  router.post("/register", async (req, res) => {
    ...
    const configuredAdmin = config.adminEmails.includes(
      userEmail.toLowerCase(),
    );
    const isAdmin =
      configuredAdmin || (config.allowFirstUserAdmin && userCount === 0);
    ...
    await db.run(
      "INSERT INTO auth_tokens (user_id, type, token, expires_at) VALUES (?, ?, ?, ?)",
      user.id,
      "email_verify",
      verifyTokenHash,
      futureDate(24 * 60),
    ...
    res.status(201).json({
      user: await ownMember(db, user),
      verification: emailResponse(sendResult, verifyToken),
    });
  });
  ...
  router.post("/password-reset/confirm", async (req, res) => {
    ...
    const token = await db.get(
      "SELECT * FROM auth_tokens WHERE token = ? AND type = ? AND used_at IS NULL",
      tokenHash,
      "password_reset",
    );
  ...
  router.post("/2fa/setup", authRequired, async (req, res) => {
    ...
    if (current?.two_factor_enabled)
      return res.status(409).json({
        error:
          "Disable two-factor authentication before setting up a new authenticator",
      });
    const secret = createTotpSecret();
    await db.run(
      "UPDATE users SET mfa_secret = ?, two_factor_enabled = 0 WHERE id = ?",
      encryptSensitive(secret, config.dataEncryptionKey),
      req.user.id,
    );
  ...
  router.post("/2fa/disable", authRequired, async (req, res) => {
    ...
    if (
      !user.mfa_secret ||
      !verifyTotp(
        decryptSensitive(user.mfa_secret, config.dataEncryptionKey),
        parsed.data.code,
      )
    ) {
      return res.status(400).json({ error: "Invalid authenticator code" });
    ...
    await db.run(
      "UPDATE users SET two_factor_enabled = 0, mfa_secret = NULL WHERE id = ?",
      user.id,
    );
    res.json({ ok: true });
  });
