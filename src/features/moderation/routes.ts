...
export function createModerationRouter({ db }) {
  ...
  router.post("/reports/posts/:id", memberRequired, async (req, res) => {
    ...
    if (!post || (await isBlockedEitherWay(db, req.user.id, post.user_id)))
  ...
  router.post("/reports/users/:id", memberRequired, async (req, res) => {
    ...
    const target = await db.get(
      "SELECT id FROM users WHERE id = ? AND is_suspended = 0",
      req.params.id,
    );
  ...
  router.get("/admin/posts", adminRequired, async (req, res) => {
    ...
    if (hideFilter === "1") {
      where += " AND posts.is_hidden = 1";
    } else if (hideFilter === "0") {
      where += " AND posts.is_hidden = 0";
    }
  ...
  router.patch("/admin/posts/:id", adminRequired, async (req, res) => {
    ...
    await db.run(
      "UPDATE posts SET body = ?, edited = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
      body,
      post.id,
    );
  ...
  router.patch("/admin/users/:id", adminRequired, async (req, res) => {
    ...
    const username =
      req.body.username !== undefined
        ...
        : user.username;
    const email =
      req.body.email !== undefined ? String(req.body.email).trim() : user.email;
    if (
      username.length < 3 ||
      username.length > 24 ||
    ...
    const isAdmin =
      req.body.is_admin !== undefined
        ...
        : user.is_admin;
    const isSuspended =
      req.body.is_suspended !== undefined
    ...
    await db.run(
      "UPDATE users SET username = ?, email = ?, bio = ?, is_admin = ?, is_suspended = ? WHERE id = ?",
      username,
      email,
      bio,
      isAdmin,
    ...
    if (Number(isAdmin) !== user.is_admin)
      changed.push(`is_admin:${isAdmin ? "promote" : "demote"}`);
    if (Number(isSuspended) !== user.is_suspended)
      changed.push(`suspend:${isSuspended}`);
  ...
  router.post("/admin/bulk", adminRequired, async (req, res) => {
    ...
    if (ids.length > 100)
      return res.status(400).json({ error: "Max 100 items per bulk action" });
    let done = 0;
  ...
  router.post("/admin/seed/users", adminRequired, async (req, res) => {
    // Seeding is a development/test affordance; never allow it to pollute a
    // production database with generated accounts.
    if (req.app.locals.context?.config?.isProduction) {
      return res
        .status(403)
        .json({ error: "Seeding is disabled in production" });
    }
  ...
  router.post("/admin/users", adminRequired, async (req, res) => {
    ...
    if (!username || !email || !password) {
      return res
        .status(400)
        .json({ error: "Username, email, and a strong password are required" });
    }
    if (username.length < 3 || username.length > 24)
      return res
        .status(400)
        .json({ error: "Username must be 3-24 characters" });
