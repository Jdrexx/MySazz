...
export function createUsersRouter({ db, config }) {
  ...
  router.get("/me", authRequired, async (req, res) =>
    res.json({ user: await ownMember(db, req.user) }),
  );
  ...
  router.patch("/me", memberRequired, async (req, res) => {
    ...
    await db.run(
      "UPDATE users SET bio = ?, avatar_url = ?, cover_url = ? WHERE id = ?",
      bio,
      avatarUrl,
      coverUrl,
      req.user.id,
    ...
    `,
      ...
      JSON.stringify(
        parsed.data.connection_intents ??
          parseStringArray(current.connection_intents),
      ),
      JSON.stringify(
        parsed.data.experience_tags ??
          parseStringArray(current.experience_tags),
      ),
      field("city"),
      field("region"),
      field("postal_code"),
      field("search_radius_miles", 25),
      field("discoverable", true) ? 1 : 0,
      field("show_relationship_status", false) ? 1 : 0,
      field("show_experience_tags", false) ? 1 : 0,
      field("presence_status", "offline"),
      req.user.id,
    );
  ...
  router.post("/me/avatar", memberRequired, async (req, res) => {
    ...
    const media = await db.get(
      "SELECT * FROM media WHERE id = ? AND user_id = ? AND mime_type LIKE ?",
      mediaId,
      req.user.id,
      "image/%",
    );
    if (!media)
      return res
        .status(400)
        .json({ error: "Upload an image first and pass its media_id" });
    await db.run(
  ...
  router.get("/me/export", memberRequired, async (req, res) => {
    const [profile, consents, posts, comments, connections, rawMessages] =
      await Promise.all([
        db.get("SELECT * FROM member_profiles WHERE user_id = ?", req.user.id),
        db.all(
          "SELECT consent_type, document_version, accepted_at, withdrawn_at FROM user_consents WHERE user_id = ? ORDER BY accepted_at",
          req.user.id,
        ),
        db.all(
          "SELECT id, body, image_url, created_at, updated_at FROM posts WHERE user_id = ? ORDER BY created_at",
          req.user.id,
        ),
        db.all(
          "SELECT id, post_id, body, created_at FROM comments WHERE user_id = ? ORDER BY created_at",
          req.user.id,
        ),
        db.all(
          "SELECT id, requester_id, recipient_id, status, created_at, updated_at FROM connections WHERE requester_id = ? OR recipient_id = ? ORDER BY created_at",
          req.user.id,
          req.user.id,
        ),
        db.all(
          "SELECT id, sender_id, recipient_id, body, read_at, created_at FROM messages WHERE sender_id = ? OR recipient_id = ? ORDER BY created_at",
          req.user.id,
          req.user.id,
        ),
      ]);
    const exported = {
      ...
      messages: rawMessages.map((message) => ({
        ...message,
        body: decryptSensitive(message.body, config.dataEncryptionKey),
      })),
    };
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="mysazz-data-export.json"',
    );
    res.json(exported);
  });
  ...
  router.get("/users/:username", memberRequired, async (req, res) => {
    ...
    if (
      user.id !== req.user.id &&
      (await isBlockedEitherWay(db, req.user.id, user.id))
    )
      return res.status(404).json({ error: "User not found" });

    const follower_count = Number(
      (
        await db.get(
    ...
    const member =
      user.id === req.user.id
        ? await ownMember(db, user)
        : await publicMember(db, user);
    if (!member) return res.status(404).json({ error: "User not found" });
    res.json({
      user: { ...member, follower_count, following_count, following },
      posts: await getPosts(db, req.user?.id, "WHERE posts.user_id = ?", [
        user.id,
      ]),
    });
  });
  ...
  router.post("/users/:username/follow", memberRequired, async (req, res) => {
    ...
    if (await isBlockedEitherWay(db, req.user.id, target.id))
      return res.status(404).json({ error: "User not found" });
