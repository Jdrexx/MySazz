...
export function createApp({ db, jwtSecret, config }: any = {}) {
  ...
  app.use(
    ...
    rateLimit({
      windowMs: 15 * 60 * 1000,
      limit: runtimeConfig.isProduction ? 30 : 100,
  ...
  app.use(
    ...
    rateLimit({
      windowMs: 60 * 1000,
      limit: runtimeConfig.isProduction ? 120 : 300,
      standardHeaders: true,
      legacyHeaders: false,
    }),
  );
  ...
  app.use(
    "/uploads",
    uploadsAuth,
    express.static(path.resolve(runtimeConfig.uploadDir), {
      index: false,
      fallthrough: false,
    }),
  );
  ...
  app.get("/api/health", (_req, res) =>
    res.json({
      ...
      realtime: {
        transport: "socket.io",
        capabilities: ["messages", "typing", "video-call-signaling"],
      },
      production: runtimeConfig.isProduction,
    }),
  );
  registerFeatures(app, context);
  // Express 5 requires named wildcards ('*splat') — '/api/*' throws at mount.
