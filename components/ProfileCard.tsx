...
export default function ProfileCard({ user, onUserUpdate }) {
  ...
  async function startMfa() {
    try {
      ...
      setMfaSetup(data);
      setStatus(
        "Add the secret to your authenticator app, then confirm a code.",
      );
    } catch (error) {
      setStatus(error.message);
    }
  ...
  return (
    <section className="card profilePanel">
      <div className="cover">
        {user.cover_url && <img src={user.cover_url} alt="Profile cover" />}
      </div>
      <Avatar user={user} size={72} />
      <div>
        <h2>@{user.username}</h2>
        <span
          className={`presence presence-${profile.presence_status || "offline"}`}
        >
          {label(profile.presence_status || "offline")}
        </span>
      </div>
      ...
      <div className="tagList">
        {(profile.connection_intents || []).map((item) => (
          <span key={item}>{label(item)}</span>
        ))}
      </div>

      {!editMode ? (
        <button className="secondaryButton" onClick={() => setEditMode(true)}>
          Edit member profile
        </button>
      ) : (
        <form onSubmit={saveProfile} className="gridForm profileEditForm">
          ...
          <label>
            Presence
            <select
              value={profileForm.presence_status}
              onChange={(event) =>
                setProfileForm({
                  ...profileForm,
                  presence_status: event.target.value,
                })
              }
            >
          ...
          <fieldset>
            <legend>Connection interests</legend>
            <div className="choiceGrid compactChoices">
              {intentOptions.map((item) => (
                <label className="checkChoice" key={item}>
                  <input
                    type="checkbox"
                    checked={profileForm.connection_intents.includes(item)}
                    onChange={() => toggleArray("connection_intents", item)}
                  />
          ...
          <fieldset>
            <legend>Lived-experience tags</legend>
            <div className="choiceGrid compactChoices">
              {experienceOptions.map((item) => (
                <label className="checkChoice" key={item}>
                  <input
                    type="checkbox"
                    checked={profileForm.experience_tags.includes(item)}
                    onChange={() => toggleArray("experience_tags", item)}
                  />
                  <span>{label(item)}</span>
                </label>
              ))}
          ...
          <div className="locationFields">
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setProfileForm({ ...profileForm, region: event.target.value })
                }
              />
            </label>
          </div>
          <label>
            Private ZIP or postal code
            <input
            ...
            />
            <small>
              Used for future resource search; never shown to members.
            </small>
          </label>
          <label>
            Search radius: {profileForm.search_radius_miles} miles
            <input
              type="range"
              min="1"
              max="100"
              value={profileForm.search_radius_miles}
              onChange={(event) =>
                setProfileForm({
                  ...profileForm,
                  search_radius_miles: Number(event.target.value),
                })
              }
            />
          </label>
          <label className="checkChoice">
            ...
            />
            <span>Let members find my profile</span>
          </label>
          <label className="checkChoice">
            <input
              type="checkbox"
              checked={profileForm.show_relationship_status}
              onChange={(event) =>
                setProfileForm({
                  ...profileForm,
                  show_relationship_status: event.target.checked,
                })
              }
            />
          ...
          <label className="checkChoice">
            <input
              type="checkbox"
              checked={profileForm.show_experience_tags}
              onChange={(event) =>
                setProfileForm({
                  ...profileForm,
                  show_experience_tags: event.target.checked,
                })
              }
            />
            <span>Show experience tags</span>
          </label>
      ...
      <details>
        ...
        <div className="gridForm detailsBody">
          <label>
            ...
            <input
              ...
              onChange={(event) =>
                ...
                setProfileImage("avatar", event.target.files[0])
              }
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={(event) =>
                event.target.files?.[0] &&
                setProfileImage("cover", event.target.files[0])
              }
            />
          </label>
          {!pwMode ? (
            <button className="secondaryButton" onClick={() => setPwMode(true)}>
              Change password
            </button>
          ) : (
            <form onSubmit={changePassword} className="gridForm">
              <input
                type="password"
                placeholder="Current password"
                value={pwForm.current_password}
                onChange={(event) =>
                  setPwForm({ ...pwForm, current_password: event.target.value })
                }
                required
              />
              <input
                ...
                onChange={(event) =>
                  setPwForm({ ...pwForm, new_password: event.target.value })
                }
                required
                minLength={8}
              />
          ...
          {!user.two_factor_enabled && !mfaSetup && (
            <button className="secondaryButton" onClick={startMfa}>
              Enable authenticator 2FA
            </button>
          )}
          {user.two_factor_enabled && (
            <form className="gridForm" onSubmit={disableMfa}>
              <p className="securityGood">Authenticator 2FA is enabled.</p>
              <input
                type="password"
              ...
              <input
                ...
                required
              />
              <button className="secondaryButton">Disable 2FA</button>
            </form>
          )}
          {mfaSetup && (
            <form className="gridForm" onSubmit={confirmMfa}>
              <p>
                Authenticator secret: <code>{mfaSetup.secret}</code>
              </p>
              <input
                inputMode="numeric"
                pattern="[0-9]{6}"
          ...
          <button
            className="secondaryButton"
            onClick={() => window.location.assign("/api/me/export")}
          >
            Download my data
          </button>
          {!deleteMode ? (
            <button className="danger" onClick={() => setDeleteMode(true)}>
              Delete my account
            </button>
          ) : (
            <form className="gridForm dangerZone" onSubmit={deleteAccount}>
              ...
              <input
                ...
                required
              />
              <button className="danger">Permanently delete account</button>
            </form>
          )}
        </div>
      </details>
