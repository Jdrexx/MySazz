...
export default function MemberOnboarding({ onComplete }) {
  ...
  async function submit(event) {
    ...
    try {
      const data = await api("/api/me/onboarding", {
        method: "POST",
  ...
  return (
    <section className="card authCard">
      ...
      <p className="formIntro">
        We updated our membership commitments. Please review and accept them
        before continuing.
      </p>
      <form className="gridForm" onSubmit={submit}>
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              setForm({ ...form, relationship_status: event.target.value })
            }
          >
        ...
        <label>
          ...
          <select
            value={form.connection_intents[0]}
            onChange={(event) =>
              setForm({ ...form, connection_intents: [event.target.value] })
            }
          >
        ...
        <div className="consentList">
          <label className="checkChoice">
            <input
              type="checkbox"
              required
              checked={form.age_18_plus}
              onChange={(event) =>
                setForm({ ...form, age_18_plus: event.target.checked })
              }
            />
          ...
          <label className="checkChoice">
            <input
              type="checkbox"
              required
              checked={form.recovery_one_year}
              onChange={(event) =>
                setForm({ ...form, recovery_one_year: event.target.checked })
              }
            />
            <span>
              I self-attest to at least one year of positive recovery progress.
            </span>
          </label>
          <label className="checkChoice">
            <input
              type="checkbox"
              required
              checked={form.member_confidentiality}
              onChange={(event) =>
                setForm({
                  ...form,
            ...
            />
            <span>I will protect member privacy and confidentiality.</span>
          </label>
          <label className="checkChoice">
            <input
              type="checkbox"
              required
              checked={form.not_medical_care}
              onChange={(event) =>
                setForm({ ...form, not_medical_care: event.target.checked })
              }
            />
            <span>MySazz does not replace professional care.</span>
          </label>
          <label className="checkChoice">
            <input
              type="checkbox"
          ...
          <label className="checkChoice">
            <input
              ...
              onChange={(event) =>
                setForm({ ...form, privacy_accepted: event.target.checked })
              }
            />
            <span>
              I accept the{" "}
