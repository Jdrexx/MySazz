...
function renderAuth() {
  $("current-user").textContent = state.user
    ? `@${state.user.username}${state.user.email_verified ? " ✓" : ""}`
    : "Guest";
...
function mediaMarkup(post) {
  ...
  if (safe.match(/\.(mp4|webm|mov)$/i))
    return `<video class="post-image" src="${safe}" controls></video>`;
...
async function loadNotifications() {
  ...
  $("notifications").innerHTML =
    data.notifications
      .map(
        (n) =>
          `<div class="mini ${n.read ? "" : "unread"}">${escapeHtml(n.body)}<br><small>${new Date(n.created_at).toLocaleString()}</small></div>`,
      )
...
async function loadThreads() {
  ...
  $("threads").innerHTML =
    data.threads
      .map(
        (t) =>
          `<button data-chat="${escapeHtml(t.username)}">@${escapeHtml(t.username)}</button>`,
      )
      .join("") || '<p class="empty">No message threads.</p>';
}
...
async function submitAuthForm(e, path, success) {
  ...
  try {
    state.user = (
      await api(path, {
        method: "POST",
        body: JSON.stringify(formObject(formEl)),
      })
    ).user;
    formEl.reset();
...
$("register-form").addEventListener("submit", (e) =>
  submitAuthForm(e, "/api/auth/register", "Account created."),
);
$("login-form").addEventListener("submit", (e) =>
  submitAuthForm(e, "/api/auth/login", "Logged in."),
);
...
$("reset-request-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    setMessage(`Reset token: ${data.dev_token || "check email"}`, "success");
  } catch (err) {
    setMessage(err.message, "error");
  }
});
...
$("reset-confirm-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    setMessage("Password reset. Log in with the new password.", "success");
  } catch (err) {
    setMessage(err.message, "error");
  }
});
...
$("verify-request-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    setMessage(`Verification token: ${data.dev_token}`, "success");
  } catch (err) {
    setMessage(err.message, "error");
  }
});
...
$("search-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    $("search-results").innerHTML =
      `<h3>Users</h3>${data.users.map((u) => `<button data-profile="${escapeHtml(u.username)}">@${escapeHtml(u.username)}</button>`).join("") || "<p>None</p>"}<h3>Posts</h3>${data.posts.map((p) => `<p>@${escapeHtml(p.username)}: ${escapeHtml(p.body)}</p>`).join("") || "<p>None</p>"}`;
...
$("feed").addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (reportId) {
      ...
      if (reason) {
        await api(`/api/reports/posts/${reportId}`, {
          method: "POST",
          body: JSON.stringify({ reason }),
        });
        setMessage("Report submitted.", "success");
      }
    }
    if (username) {
      ...
      $("view-title").textContent =
        `@${profile.user.username} — ${profile.user.follower_count} followers`;
      if (state.user && state.user.username !== profile.user.username) {
        $("follow-target").textContent = profile.user.following
...
$("follow-target").addEventListener("click", async (e) => {
  try {
    await api(`/api/users/${e.target.dataset.username}/follow`, {
      method: "POST",
    });
...
$("message-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await api(`/api/messages/${data.username}`, {
      method: "POST",
...
$("threads").addEventListener("click", async (e) => {
  ...
  try {
    ...
    $("threads").innerHTML = data.messages
      .map(
        (m) =>
          `<p><strong>@${escapeHtml(m.sender_username)}</strong>: ${escapeHtml(m.body)}</p>`,
      )
      .join("");
  } catch (err) {
    setMessage(err.message, "error");
  ...
  try {
    ...
    $("admin-reports").innerHTML =
      data.reports
        .map(
          (r) =>
            `<div class="mini">${escapeHtml(r.target_type)} #${r.target_id}: ${escapeHtml(r.reason)} <button data-remove-post="${r.target_id}">Hide post</button></div>`,
        )
        .join("") || "<p>No reports.</p>";
  } catch (err) {
    setMessage(err.message, "error");
...
$("admin-reports").addEventListener("click", async (e) => {
  ...
  try {
    ...
    setMessage("Post hidden.", "success");
    $("load-admin").click();
    await loadFeed();
  } catch (err) {
    setMessage(err.message, "error");
